...
public class VideoCodec extends Thread {
    ...
    public void startLive(MediaProjection mediaProjection) {
        File out264File = new File(App.getInstance().getExternalFilesDir(Environment.DIRECTORY_MOVIES), "screen.h264");
        try {
    ...
    private void initVirtualDisplay() {
        virtualDisplay = mediaProjection.createVirtualDisplay(
                ...
                width,
                height,
                App.getInstance().getResources().getDisplayMetrics().densityDpi, // virtualDisplay 的 dpi 值，这里都跟应用保持一致即可
                // 表示是一个虚拟的surface
